...
def _mixar_patch_icon_value_for_blender_enums():
    ...
    # class returns the bare function (no `__func__`), so just capture
    # that directly — calling it with `(icon_name)` matches the original
    # signature.
    _orig_loader = ToolSelectPanelHelper._icon_value_from_icon_handle

    # Cache the UILayout `icon=` enum so repeated lookups don't re-walk
    # `bl_rna` every call.
    _enum_items = None

    def _get_enum_items():
        nonlocal _enum_items
        if _enum_items is None:
            try:
                _enum_items = (
    ...
    def _icon_value_from_icon_handle(icon_name):
        if icon_name and icon_name[0].isupper():
            items = _get_enum_items()
            try:
                item = items[icon_name]
            except (KeyError, TypeError):
                item = None
            if item is not None:
                return item.value
        return _orig_loader(icon_name)
...
class _defs_mixar_uv:
    ...
    def uv_tool():
        ...
        return dict(
            ...
            # it lives in the same vector-padded format.
            icon="ops.uv_tool",
            widget=None,
            keymap=None,
            options={'KEYMAP_FALLBACK'},
    ...
    def uv_functions():
        ...
        return dict(
            ...
            # at the same scale as the neighbouring tool icons.
            icon="ops.uv.mark_seam",
            widget=None,
            keymap=None,
            options={'KEYMAP_FALLBACK'},
        )
...
class IMAGE_PT_tools_active(ToolSelectPanelHelper, Panel):
    ...
    # Set by `draw()` for the lifetime of the toolbar render pass so
    # `tools_from_context` filters out the hidden header-mode anchor
    # `builtin.uv_panel_mode`. Lookups from outside the draw pass
    # (e.g. `_active_panel_update`'s call to `wm.tool_set_by_id`) see
    # an unfiltered list and resolve the tool normally.
    _hide_panel_mode_in_toolbar = False
    ...
    def tools_from_context(cls, context, mode=None):
        ...
        hide_panel_mode = cls._hide_panel_mode_in_toolbar
        for tools in (cls._tools[None], cls._tools.get(mode, ())):
            for item in tools:
                if not (type(item) is ToolDef) and callable(item):
                    ...
                    # flag is False.
                    if (hide_panel_mode
                            and isinstance(item, ToolDef)
                            and item.idname == 'builtin.uv_panel_mode'):
                        continue
                    yield item

    def draw(self, context):
        cls = type(self)
        cls._hide_panel_mode_in_toolbar = True
        try:
            cls.draw_cls(self.layout, context)
        finally:
            cls._hide_panel_mode_in_toolbar = False
    ...
    _tools_transform = (
        # _defs_image_uv_transform.translate,  # Move - Commented out per user request
        # _defs_image_uv_transform.rotate,     # Rotate - Commented out per user request
        # _defs_image_uv_transform.scale,      # Scale - Commented out per user request
        _defs_image_uv_transform.transform,     # Transform - Only this tool is visible
    )
    ...
    _tools = {
        ...
        'MIXAR_UV': [
            ...
            *_tools_transform,
            ...
            None,
            # Annotate tools — pushed to the bottom of the toolbar.
            *_tools_annotate,
            # Header-tab anchor tool. The workspace tool gets switched
            # to `builtin.uv_panel_mode` whenever any of the seven
            # header tabs (UV Set / Unwrap / Texel / Layout / Image /
            # Material / Export) is clicked, so the six real toolbar
            # buttons above all show as un-depressed at the same time —
            # giving the artist exactly one visually-selected option
