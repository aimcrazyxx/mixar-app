...
class TOPBAR_MT_file(Menu):
    ...
    def draw(self, context):
        ...
        props = layout.operator("wm.read_homefile", text="New", icon='FILE_NEW')
        props.app_template = ""
...
class TOPBAR_MT_file_new(Menu):
    ...
    def draw_ex(layout, _context, *, use_splash=False, use_more=False):
        ...
        # Filter to only show Blender_Legacy in splash
        if use_splash:
            paths = [p for p in paths if p == "Blender_Legacy"]
        splash_limit = 6
...
class TOPBAR_MT_file_export(Menu):
    ...
    def draw(self, _context):
        # Add Blend file export at the top of the menu
