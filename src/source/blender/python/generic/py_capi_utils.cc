...
bool PyC_IsInterpreterActive()
{
   ...
   * for null answered "does *any* thread hold the GIL". That let the main thread
   * (GIL released around #WM_operator_call_py) run Python C-API concurrently with
   * a background Python thread, corrupting the allocator (SIGSEGV in
   * #PyUnicode_New via #BPY_context_member_get). #PyGILState_Check is per-thread. */
  if (!Py_IsInitialized()) {
    return false;
  }
