 ...
 */
static bool wm_window_is_alive(const wmWindowManager *wm, const wmWindow *win)
{
  return BLI_findindex(&wm->windows, win) != -1;
}
...
void wm_event_do_handlers(bContext *C)
{
  ...
  for (wmWindow &win : wm->windows) {
    ...
    while ((event = static_cast<wmEvent *>(win.runtime->event_queue.first))) {
      ...
      /* File-read case, or a handler closed `win` itself. */
      if (CTX_wm_window(C) == nullptr || !wm_window_is_alive(wm, win)) {
      ...
      if ((action & WM_HANDLER_BREAK) == 0) {
        ...
        ED_screen_areas_iter (&win, screen, area) {
          ...
          if (wm_event_inside_rect(event, &area->totrct)) {
            ...
            if (area->spacetype == SPACE_AGENT_BUBBLE && event->type == RIGHTMOUSE) {
              action |= WM_HANDLER_BREAK;
            }
            else {
            ...
            /* File-read case (Python), #29489, or a handler closed `win` itself. */
            if (CTX_wm_window(C) == nullptr || !wm_window_is_alive(wm, win)) {
              wm_event_free_and_remove_from_queue_if_valid(event);
              GPU_render_end();
              return;
