...
static std::vector<wmWindow *> wm_agent_bubble_windows_unlink_for_write(wmWindowManager *wm)
{
  std::vector<wmWindow *> removed_windows;
  if (wm == nullptr) {
    return removed_windows;
  }

  bool has_non_bubble_window = false;
  LISTBASE_FOREACH (const wmWindow *, win, &wm->windows) {
    if (!wm_window_contains_agent_bubble_space(win)) {
      has_non_bubble_window = true;
      break;
    }
  }
  if (!has_non_bubble_window) {
    return removed_windows;
  }
  ...
  LISTBASE_FOREACH_MUTABLE (wmWindow *, win, &wm->windows) {
    ...
    removed_windows.push_back(win);
  }

  return removed_windows;
}

static void wm_agent_bubble_windows_relink_after_write(
    wmWindowManager *wm, const std::vector<wmWindow *> &removed_windows)
{
  if (wm == nullptr) {
    return;
  }

  for (wmWindow *win : removed_windows) {
...
>>>>>>> /tmp/tmps6tg1s6n/modified
  ...
  if (!has_match) {
    ...
    LISTBASE_FOREACH (wmWindow *, win, &old_wm->windows) {
      if (!wm_window_contains_agent_bubble_space(win)) {
        win_old = win;
        break;
      }
    }
    wm_file_read_setup_wm_substitute_old_window(old_wm, wm, win_old, win_new);
  }
 ...
 *
 * Runs after #wm_file_read_setup_wm_finalize's window matching and before the
 * first #WM_check, so the stripped windows never get a GHOST window. Only
 * windows with `ghostwin == nullptr` are closed — a live bubble of the
 * current session (kept-old WM when loading without UI) is never touched.
 */
static void wm_file_read_strip_agent_bubble_windows(bContext *C, wmWindowManager *wm)
{
  if (wm == nullptr) {
    return;
  }
  /* In background mode no GHOST windows are ever created, so deserialized
   * bubble windows are inert data — and every window has a null `ghostwin`,
   * making "never materialized" undecidable. Leave the data untouched (the
   * write-path unlink still keeps them out of any file saved headlessly);
   * same guard as upstream's #wm_window_ghostwindows_remove_invalid. */
  if (G.background) {
    return;
  }

  /* Never close the only remaining window(s); a file that somehow contains
   * nothing but bubble windows must keep at least one usable window. */
  bool has_non_bubble_window = false;
  LISTBASE_FOREACH (wmWindow *, win, &wm->windows) {
    if (!wm_window_contains_agent_bubble_space(win)) {
      has_non_bubble_window = true;
  ...
  /* #wm_window_close → #ED_region_exit unconditionally dereferences the
   * message bus (#WM_msgbus_clear_by_owner has no null guard), but for a
   * freshly read WM the bus is only created by the first #WM_check — which
   * deliberately runs after this strip. Create it here exactly as #WM_check
   * would; WM_check keeps it (it only creates the bus when null). */
  if (wm->runtime->message_bus == nullptr) {
    wm->runtime->message_bus = WM_msgbus_create();
  }

  /* #wm_window_close mutates the window list (it recursively closes child
   * windows, e.g. the pill whose parent is the bubble), so restart the
   * iteration after every close. It also handles the temp-screen / layout
   * cleanup and is explicitly designed to cope with improperly initialized
   * windows — the same call upstream uses in
...
static void wm_filepath_default(const Main *bmain, char *filepath, const char *extension = ".mixar")
{
  if (bmain->filepath[0] == '\0') {
    ...
    SNPRINTF(filename_untitled, "%s%s", DATA_("Untitled"), extension);
...
static void save_set_filepath(bContext *C, wmOperator *op)
{
  ...
  if (!RNA_property_is_set(op->ptr, prop)) {
    ...
    if (ext_prop && RNA_property_is_set(op->ptr, ext_prop)) {
      ...
      if (file_ext_enum == 1) { // BLEND
        target_extension = ".blend";
      }
    }
