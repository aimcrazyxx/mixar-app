...
/**
 * Retry a file operation up to `max_retries` times with a short sleep between attempts.
 * Needed on Windows where antivirus, IDE indexers, or Explorer can hold file locks briefly.
 * \return 0 on success, -1 if all retries exhausted.
 */
static int remove_with_retry(const char *filepath, int max_retries)
{
  for (int attempt = 0; attempt <= max_retries; attempt++) {
    ...
    if (errno != EACCES && errno != EBUSY) {
      /* Not a transient locking error, bail out immediately. */
      return -1;
    }
#ifdef _WIN32
    Sleep(100); /* 100 ms between retries. */
#endif
  }
  return -1;
}

static int rename_with_retry(const char *src, const char *dst, int max_retries)
{
  for (int attempt = 0; attempt <= max_retries; attempt++) {
    if (rename(src, dst) == 0) {
      return 0;
...
#define REN_IF_DIFF \
  { \
    ...
    if (file_test) { \
      ...
      if (remove_with_retry(orgfile, 5) != 0) { \
        CLOG_ERROR(&LOG, "remove error (%s): \"%s\"", strerror(errno), orgfile); \
        return -1; \
      } \
    } \
  } \
  if (rename_with_retry(tmpfile, orgfile, 5) != 0) { \
...
static void make_bad_file(const char *file, int line)
{
  ...
  if (!fp) {
    fprintf(stderr,
            "Error: cannot create error marker file \"%s\" (from %s:%d): %s\n",
            file,
            __FILE__,
            line,
            strerror(errno));
