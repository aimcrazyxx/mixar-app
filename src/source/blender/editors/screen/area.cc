    ...
    ScrArea *area, ARegion *region, rcti *remainder, rcti *overlap_remainder, int quad)
{
  ...
  if (region->regiontype == RGN_TYPE_HEADER) {
     ...
     * a strip of empty area beneath it that renders the area
     * background — visible to the user as a darker rectangle
     * inside the lighter pill ("two backgrounds"). When the area
     * has explicitly set sizey above the default header size, we
     * honour it instead. The main bubble's HEADER keeps its
     * default sizey so this branch is a no-op there. */
    if (area->spacetype == SPACE_AGENT_BUBBLE &&
        region->sizey > 1 &&
