...
/* -------------------------------------------------------------------- */
/** \name Multi-Line Text Input Helpers
 * \{ */
...
/**
 * Get the wrapped lines and their byte offsets for a multi-line text button.
 * \param but: The text button
 * \param out_lines: Filled with wrapped line StringRefs
                                      ...
                                      bool select)
{
  ...
  for (int i = 0; i < num_lines; i++) {
    const int line_start = byte_offsets[i];
    const int next_start = (i + 1 < num_lines) ? byte_offsets[i + 1]
                                                : (int(strlen(but->editstr)) + 1);
  ...
  const int local_pos = std::min(but->pos - byte_offsets[cursor_line],
                                 int(lines[cursor_line].size()));
  const float cursor_x = BLF_width(fontid,
                                    but->editstr + byte_offsets[cursor_line],
                                    local_pos);

  /* Find byte offset at same X on target line */
  const int target_line_start = byte_offsets[target_line];
  const int target_line_len = int(lines[target_line].size());
  const int target_offset = BLF_str_offset_from_cursor_position(
      fontid, but->editstr + target_line_start, target_line_len, int(cursor_x));

  const int new_pos = target_line_start + target_offset;

  if (select) {
    /* If no selection started yet, anchor at current position */
    if (but->selsta == but->selend) {
      but->selsta = but->pos;
      but->selend = but->pos;
    }
    /* Maintain selection anchor - we know sel_pos_init is the anchor */
    but->pos = short(new_pos);
    /* Recalculate selection range */
    const int anchor = text_edit.sel_pos_init;
    but->selsta = short(min_ii(anchor, new_pos));
    but->selend = short(max_ii(anchor, new_pos));
  }
  else {
    but->pos = short(new_pos);
...
static bool textedit_insert_buf(Button *but, TextEdit &text_edit, const char *buf, int buf_len)
{
  ...
  /* Safety: clamp cursor and selection to valid string bounds to prevent
   * out-of-bounds memmove/memcpy if cursor positioning produced a bad offset. */
  CLAMP(but->pos, 0, len);
  CLAMP(but->selsta, 0, len);
  CLAMP(but->selend, 0, len);
...
>>>>>>> /tmp/tmpdb2g3731/modified
          ...
          /* Event consumed here: this prevents the same Ctrl/Cmd+V press from
           * propagating to the region keymap handler in the same frame. */
          retval = WM_UI_HANDLER_BREAK;
        }
        break;
