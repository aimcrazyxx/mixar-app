                                         ...
                                         const size_t end)
{
  ...
  for (const ColorItem &item : items) {
    ...
    /* Replace stop-color attributes for gradient stops */
    att_start = start;
    while (true) {
      constexpr static blender::StringRef key = "stop-color:";
      att_start = svg.find(key, att_start);
      if (att_start == std::string::npos || att_start > end) {
        break;
      }
...
static void icon_source_edit_cb(std::string &svg)
{
  ...
  /* Scan string, processing only groups and stops with our keyword ids. */
  ...
  while (true) {
    ...
    /* Check if this is a stop element (for gradients) */
    const size_t stop_check = svg.rfind("<stop", id_start);
    ...
    if (stop_check != std::string::npos &&
      ...
      /* This is a stop element - find its bounds */
      element_start = stop_check;
      element_end = svg.find("/>", id_start);
      if (element_end == std::string::npos) {
        element_end = svg.find("</stop>", id_start);
      }
      if (element_end == std::string::npos) {
        /* Malformed. */
        g_start = id_start + 1;
      ...
      element_start = group_check;
      element_end = svg.find("</g>", id_start);
      if (element_end == std::string::npos) {
        /* Malformed. */
        return;
      }
      ...
      /* Malformed - no valid element found. */
      return;
    }
    ...
    if (id_end != std::string::npos) {
      ...
      /* Replace this element's colors. */
      svg_replace_color_attributes(svg, id_name, element_start, element_end);
    }
