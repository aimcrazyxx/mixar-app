...
static void image_init(wmWindowManager * /*wm*/, ScrArea *area)
{
  ...
  if (!has_channels) {
    ...
    /* Insert before the WINDOW region so that region_rect_recursive allocates
     * right-aligned space for CHANNELS before WINDOW fills the remainder.
     * BLI_addtail would place it after WINDOW, leaving zero space. */
    ARegion *region_window = BKE_area_find_region_type(area, RGN_TYPE_WINDOW);
    if (region_window) {
      BLI_insertlinkbefore(&area->regionbase, region_window, region);
    }
    else {
    ...
    region->alignment = RGN_ALIGN_RIGHT;
    region->flag = RGN_FLAG_HIDDEN;
    /* Manually assign region type since ED_area_and_region_types_init already ran */
...
static void mixar_uv_props_region_listener(const wmRegionListenerParams *params)
{
  ...
  switch (wmn->category) {
    case NC_SCENE:
      if (ELEM(wmn->data, ND_MODE, ND_OB_ACTIVE, ND_TOOLSETTINGS)) {
        ED_region_tag_redraw(region);
      }
      break;
    case NC_OBJECT:
...
void ED_spacetype_image()
{
  ...
  art->poll = image_buttons_region_poll;
