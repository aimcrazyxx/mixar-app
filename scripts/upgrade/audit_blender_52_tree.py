...
* ``--repository-mode``: scan ``src`` immediately after checkout and label the
  output accordingly. Repository sources are expected to be Blender 5.2-clean;
  compatibility fixes belong in ``src`` rather than in a build-time rewrite.
"""
...
def main() -> int:
    ...
    search_menu = root / "source/blender/editors/interface/templates/interface_template_search_menu.cc"
    if search_menu.is_file() and "SPACE_MENU_NOP(SPACE_AGENT_BUBBLE);" not in search_menu.read_text(
        encoding="utf-8", errors="replace"
    ):
        errors.append(
            "source/blender/editors/interface/templates/interface_template_search_menu.cc: "
            "missing SPACE_AGENT_BUBBLE switch coverage"
        )
    ...
    if args.repository_mode:
        print("Repository mode: source overlays must already be Blender 5.2-clean.")
